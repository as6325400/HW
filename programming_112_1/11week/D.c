...
bool insert(int num, int data, node **root){
  node *temp = *root;
  node *newnode = (node*)malloc(sizeof(node));
  newnode -> num = data;
  newnode -> next = NULL;
...
signed main(){
  ...
  while(scanf("%s", operation) != EOF){
    if(strcmp(operation, "print") == 0) print(root);
      ...
      int num, data;
      scanf("%lld%lld", &num, &data);
      bool ans = insert(num, data, &root);
      if(ans) printf("INSERT_SUCC\n");
      else printf("INSERT_FAIL\n");
    }
    else{
      int num;
      scanf("%lld", &num);
      bool ans = Delete(num, &root);
      if(ans) printf("DELETE_SUCC\n");
